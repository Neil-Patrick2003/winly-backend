...
return [
     ...
     *
     * Raised from the package default of 10MB to cover the largest clip a win
     * may carry. This is a backstop and not the rule the API answers with:
     * `App\Rules\MediaFile` holds the per-kind limit and rejects an oversized
     * upload as a validation error, long before it reaches here.
     */
     ...
     *
     * Named as a string rather than by `::class`, because Media Library Pro is
     * not installed here and there is no such class to point at. Nothing reads
     * this without that package, and it will resolve if it is ever added.
     */
     ...
     *
     * Kept to a folder of its own so the library's numbered directories do not
     * sit loose at the root of the public disk beside `avatars` and `covers`,
     * which are still written by hand.
     */
